void readEEPROM() {
  ...
  updateFlag = constrain(eepromValues[9], false, true); programFlag = (eepromValues[10], false, true);
  eeprom_read_block((void*)&wdtFlag, (void*)512, sizeof(wdtFlag));
  delay(10);  //wait for the eeprom to read

// set default values in new arduino board if the program flag != 1  
 if(eepromValues[10] != 1) {  
   long eepromDefaults[] = {300, 900, 5400, 15, 300, 1800, 20, 500, 0, 0, 1};
   for(int i=0; i<=10; i++) { 
     eepromValues[i] = eepromDefaults[i];
     eeprom_write_block((void*)&eepromValues[i], (void*)(i*4), sizeof(eepromValues[i]));  //write the value to the eeprom in the correct spot for that variable.
     delay(10);
   }
   resetFlag = true;
 } 
...
long writeEEPROM() {
  ...
  eeprom_write_block((void*)&eepromValues[index2], (void*)(index2*4), sizeof(eepromValues[index2]));  //write the value to the eeprom in the correct spot for that variable.
