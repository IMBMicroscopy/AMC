...
void PinControl() {  //turn on/off the powerboard LED and relay as required
  ...
  if(flashFlag == false) {  //if the coolLED isnt flashing turn it on 
    if(coolPinFlag == true) {
      digitalWrite(coolLED, HIGH);
    } 
    else digitalWrite(coolLED, LOW);
  }
  if(beepFlag == true) {  //beep the piezo beeper
    if((runTime - lastBeepTime) >= 1000) {
      digitalWrite(beepPin, HIGH);
      lastBeepTime = runTime;
    }
    if((runTime - lastBeepTime) >= beepLength) {
  ...
  if(flashFlag == true) {  //Flash the coolLED
    if((runTime - lastFlashTime) >= 1000) {
      digitalWrite(coolLED, HIGH);
      lastFlashTime = runTime;
    }
    if((runTime - lastFlashTime) >= flashLength) {
...
void gotoZeroTimeMode() {
  ...
  if(powerPinFlag == false && lampPinFlag == false) {
    if(updateFlag) Serial.println("goto zeroMode");
    zeroTimer = onTime();
...
void gotoMinRunMode() {
  ...
  if((lampUSBFlag == true && lampPinFlag == false && newLampCommand == true) || (powerUSBFlag == true && lampPinFlag == false && newPowerCommand == true)) {  
    if(updateFlag) Serial.println("goto minMode");
...
void gotoMaxRunMode() {
  ...
  if((lampUSBFlag == true && lampPinFlag == true && newLampCommand == true)  || (powerUSBFlag == true && lampPinFlag == true && newPowerCommand == true)) { 
    if(updateFlag) Serial.println("goto maxMode");
...
void gotoLogOffMode() {
  ...
  if(runTime >= minTime) {
    if(logOffFlag == true) {
      if(updateFlag) Serial.println("goto logOffMode");
...
void ifReset() {
  if(wdtFlag == 1) {  //if the unit was reset whilst in zeroTimeMode, go straight to zeroTimeMode
    if(updateFlag) Serial.println("unit was reset"); 
    wdtFlag = 255;
    eeprom_write_block((void*)&wdtFlag, (void*)512, sizeof(wdtFlag));
    delay(10);
    ...
    if(updateFlag) Serial.println("goto coolMode");
...
void maxResetTime() {
  if((onTime() - zeroTimer) >= resetTime) {  // reset the unit if it's not being used
    wdtFlag = 1;
    eeprom_write_block((void*)&wdtFlag, (void*)512, sizeof(wdtFlag));  //set EEPROM flag to indicate a watchdog reset was performed
    delay(10);
    if(updateFlag) Serial.println("set WDT=" + String(wdtFlag));
    delay(10);
    if(updateFlag) Serial.println("unit will now reset");
    delay(10);
    cpuReset();  
...
void Ram() {
  ...
  if(freeRam() <= 100) {
    Serial.println("running out of RAM: unit will now reset");
    delay(1000);
    cpuReset();
  }
}
